...
if __name__ == "__main__":
    # Example Usage
    sim = MonteCarloSimulator(iterations=5000)
    
    scenario = {
        "clicks": {
            "distribution": "normal",
            "params": {"mean": 1000, "std": 150}
        },
        "cpc": {
            "distribution": "normal",
            "params": {"mean": 1.50, "std": 0.20}
        },
        "conversion_rate": {
            "distribution": "normal", # simplified, beta is better for rates
    ...
    results = sim.run_simulation(scenario)
    
    revenue = results["metrics"]["revenue"]
    print(f"Projected Revenue: ${revenue['mean']:.2f} (±${revenue['std']:.2f})")
    print(f"95% Confidence Interval: ${revenue['p5']:.2f} - ${revenue['p95']:.2f}")
    
    profit_samples = (np.random.normal(1000, 150, 5000) * 0.10 * 50) - (np.random.normal(1000, 150, 5000) * 1.5)
    var_95 = sim.calculate_var(profit_samples, 0.95)
    print(f"Value at Risk (95%): ${var_95:.2f}") # Amount of money we could lose
