...
class CampaignFitnessEvaluator:
    """
    Evaluates strategy DNA against campaign goals.
    Uses historical data simulation or direct DB metrics.
    """
    def __init__(self, historical_data: Optional[Dict[str, Any]] = None, target_acos: float = 30.0):
        self.historical_data = historical_data or {}
        self.target_acos = target_acos

    def evaluate(self, dna: Dict[str, Any]) -> float:
        """
        ...
        # 1. Bid Aggressiveness vs Target ACoS
        bid_multiplier = float(dna.get("bid_multiplier", 1.0))
        target_acos_diff = self.target_acos - 30.0 # Deviation from standard
        
        # If target ACoS is low, we want lower bids
        if self.target_acos < 20:
            if bid_multiplier < 1.0: score += 10
            elif bid_multiplier > 1.2: score -= 10
        # If target is high (launch), we want aggression
        elif self.target_acos > 40:
            if bid_multiplier > 1.1: score += 10
            
        # 2. Keyword Harvesting
        harvest_enabled = dna.get("enable_harvesting", False)
        if harvest_enabled:
            score += 5 # Generally good
            
        # 3. Dayparting (Simulated logic)
        ad_schedule = dna.get("ad_schedule", "all_day")
        if ad_schedule == "peak_hours":
            # Assume peak efficiency
            score += 8
            
        # 4. Simulation using simplified Monte Carlo logic (if data available)
        if self.historical_data:
            # Simulate impact
            # This would connect to the MonteCarloSimulator in a full implementation
            simulated_cvr = self.historical_data.get("cvr_30d", 0.1) * bid_multiplier
            predicted_acos = (1.0 / simulated_cvr) * 0.5 if simulated_cvr > 0 else 100
            
            # Distance from target
            acos_error = abs(predicted_acos - self.target_acos)
            score += max(0, 40 - acos_error) # Add up to 40 points for accuracy
